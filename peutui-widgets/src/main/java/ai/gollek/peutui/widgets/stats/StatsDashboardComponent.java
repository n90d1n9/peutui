package ai.gollek.peutui.widgets.stats;
...
/**
 * TUI component that displays a dashboard of chat statistics and performance metrics.
 * Shows daily/monthly token usage, average performance metrics, and recent activity.
 * 
...
public class StatsDashboardComponent implements Component {
    ...
    public void render(RenderContext context) {
        ...
        if (today.totalCost() > 0) {
            buffer.writeAt(x, row++, "│    Cost: " + VALUE_COLOR + "$" + String.format("%.4f", today.totalCost()) + RESET +
        ...
        if (month.totalCost() > 0) {
            buffer.writeAt(x, row++, "│    Cost: " + VALUE_COLOR + "$" + String.format("%.4f", month.totalCost()) + RESET);
        }
        buffer.writeAt(x, row++, "│");
        ...
        if (metrics.sampleCount() > 0) {
            ...
            buffer.writeAt(x, row++, "│    TTFT: " + VALUE_COLOR + String.format("%.2f ms", metrics.avgTtftMs()) + RESET +
                           " | Speed: " + VALUE_COLOR + String.format("%.2f t/s", metrics.avgTokensPerSecond()) + RESET);
            buffer.writeAt(x, row++, "│    Duration: " + VALUE_COLOR + String.format("%.2f ms", metrics.avgDurationMs()) + RESET);
            buffer.writeAt(x, row++, "│    Total Generated: " + VALUE_COLOR + formatNumber(metrics.totalGeneratedTokens()) + RESET +
        ...
        // Footer
        buffer.writeAt(x, row++, "│  " + SUBTLE_COLOR + "Press 'd' for details, arrows to scroll, 'q' to close" + RESET);
        buffer.writeAt(x, row, "╰" + "─".repeat(width - 2));
    }
    
    @Override
    public boolean handleInput(InputEvent event) {
        if (event instanceof ai.gollek.peutui.core.event.KeyEvent keyEvent) {
    ...
    private static class DetailedMetricsComponent implements Component {
        ...
        public void render(RenderContext context) {
            ...
            for (int i = start; i < metrics.size(); i++) {
                ...
                buffer.writeAt(x, row++, "│    Speed: " + VALUE_COLOR + String.format("%.2f t/s", m.tokensPerSecond()) + RESET +
